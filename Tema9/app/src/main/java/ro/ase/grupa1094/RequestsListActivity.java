...
import static ro.ase.grupa1094.TipAnimal.CAT;
import static ro.ase.grupa1094.TipAnimal.DOG;
...
public class RequestsListActivity extends AppCompatActivity {
    private static final String urlAnimale = "https://jsonkeeper.com/b/MLY2";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Animal animal1 = new Animal( "miau", "654342", "hjnfjs@yahoo.com", CAT);
        AnimalDB dbInstance = AnimalDB.getInstance(getApplicationContext());
        dbInstance.getAnimalDAO().insertAnimal(animal1);

        List<Animal> listaAnimale = dbInstance.getAnimalDAO().getAnimale();
        ...
        editor.putString("token", "Request a pet!");
        editor.apply();
        ...
        initComponente();
        animalList.add(new Animal("Miau", "456789034","miau@yahoo.com",CAT));
        animalList.add(new Animal("Ham", "654342343","ham@yahoo.com",DOG));
        incarcareAnimaleDinRetea();
    }

    private void incarcareAnimaleDinRetea() {
        Thread thread = new Thread(){
            @Override
            public void run() {
                HttpsManager httpsManager = new HttpsManager(urlAnimale);
                String rezultat = httpsManager.procesare();
                new Handler(getMainLooper()).post(()->{
                    preluareAnimaleDinJson(rezultat);
                });
            }
        };
        thread.start();
    }
    private void preluareAnimaleDinJson(String json){
        animalList.addAll(AnimalParser.parsareJson(json));
        ArrayAdapter<Animal> adapter = (ArrayAdapter<Animal>) lvRequests.getAdapter();
        adapter.notifyDataSetChanged();
    }

    private void initComponente() {
        lvRequests = findViewById(R.id.lvProfileChanges);
        ArrayAdapter<Animal> adapter = new ArrayAdapter<>(getApplicationContext(), android.R.layout.simple_list_item_1, animalList);
        lvRequests.setAdapter(adapter);
    }
